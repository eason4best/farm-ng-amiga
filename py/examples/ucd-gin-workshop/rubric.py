...
async def main() -> None:
    ...
    # Step 1: Drive forward 1 meter to line it up with the row start
    print("\n--- Step 1: Driving forward 1 meter ---")
    await drive_distance(client, 1.0, LINEAR_VELOCITY)

    # Step 2: Drive forward 10 meters (row length)
    print("\n--- Step 2: Driving forward 10 meters ---")
    await drive_distance(client, 10.0, LINEAR_VELOCITY)

    # Step 5: Drive a pi turn with radius of 1 meters
    print("\n--- Step 3: Driving a pi turn with 1 meter radius ---")
    # Pi radians = 180 degrees, counterclockwise turn
    await drive_arc(client, radius=1.0, angle_radians=math.pi, linear_velocity=LINEAR_VELOCITY, direction=1)
